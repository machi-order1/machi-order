(function (global) {
  ...
  function renderHappy(data, escape, yen) {
    ...
    return `<div class="happy-heading"><div><span class="happy-eyebrow">HAPPY HOUR</span><h2>お得な一杯を、すぐ注文。</h2><p>お酒だけでもどうぞ</p></div>${ends.length === 1 ? `<b class="happy-end">${escape(ends[0])}まで</b>` : ''}</div><div class="happy-drinks">${drinks.slice(0, 3).map(product => `<div class="happy-drink"><strong>${escape(product.name)}</strong><s>通常 ${yen(product.regular_price)}</s><b class="happy-amount">${yen(product.price)}</b><small>${yen(Number(product.regular_price) - Number(product.price))}お得</small><button type="button" data-happy-product="${Number(product.id)}">${(product.option_group_ids || []).length ? '選ぶ' : 'カートに追加'}</button></div>`).join('')}</div><div class="happy-bottom"><button type="button" id="happy-view-all">対象のお酒を見る →</button><span id="happy-added" role="status" aria-live="polite"></span></div>`;
  }
