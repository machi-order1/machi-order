...
const [cashier, cashierApi, paymentUndo, reset, login, opening, operationLog, operationsApi, worker, menu, kitchen, productAdmin, menuAdminApi, menuApi, kitchenApi, orderingGuard, timeClock, myShifts, workforceApi, uiConfig] = await Promise.all([
  source('cashier.html'),
...
expect('更新キャッシュ番号', worker.includes("machi-order-v71-17"));
...
expect('勤怠・シフトをオフラインキャッシュ対象に追加', worker.includes("machi-order-v71-17") && worker.includes("'/time-clock.html'") && worker.includes("'/my-shifts.html'"));
