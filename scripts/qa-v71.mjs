...
const [cashier, cashierApi, paymentUndo, reset, login, opening, operationLog, operationsApi, today, manager, dailyProfit, laborDashboard, worker, menu, kitchen, kitchenManifest, shirakibaruKitchen, shirakibaruManifest, kitchenApps, productAdmin, menuAdminApi, menuApi, kitchenApi, orderingGuard, timeClock, myShifts, workforceApi, uiConfig, closing, closingApi, inventory, inventoryApi, systemCheck, offline, inventoryImport, inventoryParser, prep, prepMigration, staffOrder, staffOrderApi, machiApp] = await Promise.all([
  source('cashier.html'),
  ...
  source('today.html'),
  source('manager.html'),
  source('daily-profit.html'),
  source('labor-dashboard.html'),
  source('sw.js'),
  source('index.html'),
  source('kitchen.html'),
  source('kitchen.webmanifest'),
  source('kitchen-shirakibaru.html'),
  source('kitchen-shirakibaru.webmanifest'),
  source('kitchen-apps.html'),
  ...
  source('staff-order.html'),
  source('supabase/functions/staff-order-api/index.ts'),
  source('machi-app.js'),
]);
...
expect('利益・人件費画面はトークン手入力を廃止', dailyProfit.includes('machi_access_token') && laborDashboard.includes('machi_access_token') && !dailyProfit.includes('ログイントークン') && !laborDashboard.includes('ログイントークン'));
expect('利益・人件費画面は店舗別に固定', dailyProfit.includes('STORE_ID=[1,2].includes') && laborDashboard.includes('STORE_ID=[1,2].includes') && manager.includes("'?store_id='+STORE_ID"));
expect('利益精度の登録状況を画面内で更新', dailyProfit.includes('data.data_quality') && dailyProfit.includes('uncosted_products'));
...
expect('口頭注文はスタッフ認証を必須化', staffOrderApi.includes("auth.auth.getUser(token)") && staffOrderApi.includes("store_memberships"));
expect('口頭注文は店舗所属と席の店舗を照合', staffOrderApi.includes(".eq('store_id', storeId)") && staffOrderApi.includes(".eq('id', tableId)"));
expect('口頭注文を通常QRと区別して記録', staffOrderApi.includes("entry_channel: 'staff_order'") && staffOrderApi.includes("staff_order_created"));
